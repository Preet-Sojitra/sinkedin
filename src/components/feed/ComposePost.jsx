...
export default function ComposePost({ onPostCreated }) {
  ...
  const handlePostClick = async () => {
    ...
    try {
      ...
      showError(true);
      setErrorMessage("Failed to create post. ", error.message || "Please try again later.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="bg-dark-secondary border border-dark-border rounded-lg p-5 md:p-6">
      ...
      {showError && <div className="mt-3 text-sm text-red-500">{errorMessage}</div>}
      {showPositiveWarning && (
        <div className="mt-3 text-sm text-yellow-500">
          Warning: Whoa, too much sunshine here! Even unicorns are jealous. Share a flop instead!
        </div>
      )}
      <div className="flex justify-between items-center mt-4">
        <div>
          <label
            className={`flex items-center gap-2 text-sm text-light-secondary select-none ${
              isAuthenticated ? "cursor-pointer hover:text-light" : "cursor-not-allowed opacity-60"
            } transition-colors`}
          >
            <input
              type="checkbox"
              className="w-4 h-4 rounded bg-dark border-dark-border text-accent focus:ring-accent cursor-pointer"
              checked={isAnonymous}
        ...
        <button
          className={`bg-accent text-primary-foreground border-none px-6 py-2 rounded-md font-semibold text-sm transition-colors duration-200 ${
            isAuthenticated
